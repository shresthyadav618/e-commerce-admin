...
<table className="mt-6 basic w-full">
    ...
    {products && products.map((elm)=>{
        ...
        return <tr key={elm.name + 'pr'}><td>{elm.name}</td> <td className="flex gap-x-4"><Link href={`/products/edit/${elm._id}`}  className="p-4 btn flex items-center  gap-x-4">Edit  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
